  ...
  persist(
    (set, get) => ({
      ...
      addToCart: (item) =>
        set((state) => {
          const existingItemIndex = state.items.findIndex((existingItem) => {
            ...
            console.log(
              `Comparing: "${existingItem.productId}" === "${item.productId}" = ${match}`
            );
            return match;
          });
      ...
      transformApiCartItem: (apiItem: ApiCartItem): CartItem => {
        ...
        const customizationTotal = apiItem.customizations.reduce(
          (sum, customization) => {
            return (
              sum +
              customization.options.reduce(
